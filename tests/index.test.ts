...
describe('trail on custom configuration', () => {
  ...
  it('Should output GET /response-route payload', async () => {
    ...
    expect(reportFn.mock.calls[5]).toEqual(["test-id", {"trailId": "test-id", "for": "handler", "method": "GET", "payload": "{\"payload\":\"some payload\",\"items\":[{\"id\":1},{\"id\":2}]}", "reqUrl": "/response-route", "routeHandlerStage": "UNIQUE HANDLER", "type": "report"}])
